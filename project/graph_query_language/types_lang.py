...
def create_graph_from_enfa(enfa: EpsilonNFA) -> "Graph":
    ...
    return Graph(
        graph=enfa.to_networkx(), start_nodes=start_nodes, final_nodes=final_nodes
    )
...
class Graph:
    def __init__(
        self, vertexes=None, edges=None, graph=None, start_nodes=None, final_nodes=None
    ) -> None:
    ...
    def intersect(self, entity: Union["Graph", "Regex"]) -> "Graph":
        ...
        if isinstance(entity, Graph):
            enfa_r = create_non_deterministic_automaton_from_graph(
                entity.gr, entity.start_nodes, entity.final_nodes
            )
        if isinstance(entity, Regex):
            enfa_r = entity.rgx.to_epsilon_nfa().minimize()
        if enfa_r is None:
            raise InvalidArgument(
                f"arguments of different types: {type(self)} != {type(entity)} or {type(entity)} != regex"
            )
        enfa_l = create_non_deterministic_automaton_from_graph(
            self.gr, self.start_nodes, self.final_nodes
        )
    ...
    def concat(self, entity: Union["Graph", "Regex"]) -> "Graph":
        ...
        if isinstance(entity, Graph):
            regex_r = (
                create_non_deterministic_automaton_from_graph(
                    entity.gr, entity.start_nodes, entity.final_nodes
                )
                .minimize()
                .to_regex()
    ...
    def inop(self, elem) -> "Bool":
        res = (
            elem in self.get_vertices()
            or elem in self.get_labels()
            or elem in self.get_edges()
        )
        return Bool(res)
...
class Regex:
    ...
    def concat(self, regex: "Regex") -> "Regex":
        if type(self) != type(regex):
            raise InvalidArgument(
                f"arguments of different types: {type(self)} != {type(regex)}"
            )
