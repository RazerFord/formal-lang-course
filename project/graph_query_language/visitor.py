...
class Visitor(LanguageVisitor):
        ...
        for args in iterable:
            ...
            if len(args) != len(lam.args):
                raise InvalidArgument(
                    "the number of arguments and the number of parameters in the lambda are not the same"
                )
    ...
    def visitFilter(self, ctx: LanguageParser.FilterContext):
        ...
        for args in iterable:
            ...
            val = self.visitExpr(lam.body)
            if val.value or (
                isinstance(val, tp.Id) and self.memory[self.visitExpr(lam.body)].value
            ):
                result.append(args)
        return result
    ...
    def visitEqual(self, ctx: LanguageParser.EqualContext):
        ...
        return tp.Bool(
            binary_l.__class__ == binary_r.__class__ and binary_l == binary_r
        )
    ...
    def _get_by_binary(self, target):
        ...
        if target.var() is not None:
            ...
            if (
                isinstance(var, tp.Graph)
                or isinstance(var, tp.Bool)
                or isinstance(var, tp.Regex)
            ):
                return var
            raise InvalidArgument(
                f"target argument '{name}' is not a Graph, Bool or Regex"
            )
        if target.graph() is not None:
            return self.visitGraph(target.graph())
        raise InvalidArgument(f"{target.getText()} not a valid argument")
